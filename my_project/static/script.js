...
function post()
{
	...
	new_list=new_list+"]";
	$.post("/a", { name: new_list },
	function(data){
	list=data.split(",");
        alert(list);
	var color = ["#3AB745", "#B8D430",  "#029990",  "#3501CB", "#2E2C75", "#673A7E", "#CC0071", "#F80120", "#F35B20", "#FB9A00", "#FFCC00", "#FEF200"];
	l=list[0].split("");
	list[0]=l[1];
	l=list[list.length-1].split("");
	list[list.length-1]=l[1];
	i=0;
	while(i<list.length)
	{
		context.fillStyle =color[(parseInt(list[i]))];
		context.beginPath();
        	context.arc(x_cord[i],y_cord[i],radius, 0, Math.PI * 2, false);
        	context.closePath();
        	context.fill();
		i+=1;
	}
